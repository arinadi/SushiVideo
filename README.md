...
!curl -s "https://raw.githubusercontent.com/arinadi/SushiVideo/main/lint.py?v=$(date +%s)" -o lint.py && python lint.py
```

```python
# @title 🎨 Run Gradio UI Mockup (Standalone)
!curl -s "https://raw.githubusercontent.com/arinadi/SushiVideo/main/gradio_ui.py?v=$(date +%s)" -o gradio_ui.py && python gradio_ui.py
```
